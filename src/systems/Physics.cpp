...
namespace ecs {
    ...
    static void compute_gravity(const flecs::world& w) {
        ...
        // Gather bodies into arrays for pairwise iteration
        struct Row {
            flecs::entity e;
            Position* p;
            Velocity* v;
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (!is_finite(ri.p->value.x) || !is_finite(ri.p->value.y)) continue;
            if (!is_finite(ri.v->value.x) || !is_finite(ri.v->value.y)) continue;
            if (!(ri.m->value > 0.0f) || !is_finite(ri.m->value)) continue;
        }
        ...
        for (size_t i = 0; i < n; ++i) {
            rows[i].e.get_mut<Acceleration>()->value = acc[i];
        }
    }
    ...
    static void integrate(const flecs::world& w, const float dt) {
        ...
        if (const int integrator = cfg.integrator; integrator == 0) {
            ...
            // Velocity Verlet using PrevAcceleration
            // 1) x_{t+dt} = x_t + v_t*dt + 0.5*a_t*dt^2
            w.each([&](Position& p, const Velocity& v, const Acceleration& a, PrevAcceleration& a0, const Pinned& pin) {
                ...
                p.value += dx;
                // store current acceleration as previous for next phase
                a0.value = a.value;
            });

            // 2) Recompute acceleration after position update
            compute_gravity(w);

            // 3) v_{t+dt} = v_t + 0.5*(a_t + a_{t+dt})*dt
    ...
    void register_physics_systems(const flecs::world& w) {
        // Allocate required components for queries; no-op here, but helpful to ensure registration.
        // w.component<Position>();
        // w.component<Velocity>();
        // w.component<Acceleration>();
        // w.component<PrevAcceleration>();
        // w.component<Mass>();
        // w.component<Pinned>();
        // w.component<Trail>();
        // w.component<Tint>();
        // w.component<Config>();
        // w.component<Selection>();

        // Gravity: once per frame before integration.
        w.system<>().kind(flecs::OnUpdate).iter([&](flecs::iter&) {
            if (const Config& cfg = *w.get<Config>(); cfg.paused) {
                return;
            }
            compute_gravity(w);
        });
